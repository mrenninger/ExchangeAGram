...
class FilterViewController: UIViewController {
    ...
    func createUIAlertController (indexPath: NSIndexPath) {
        ...
        let photoAction = UIAlertAction(title: "Post to FB with Caption", style: .Destructive) { (UIAlertAction) -> Void in
        ...
        let savedFilterAction = UIAlertAction(title: "Save filter w/o posting to FB", style: .Default) { (UIAlertAction) -> Void in
            var text:String = tf.text
            self.saveFilterToCoreData(indexPath, caption:text)
        }
        alert.addAction(savedFilterAction)
    ...
    func saveFilterToCoreData(indexPath:NSIndexPath, caption:String) {
        let filterImage = self.filteredImageFromImage(thisFeedItem.image, filter: filters[indexPath.row])
        
        thisFeedItem.image = UIImageJPEGRepresentation(filterImage, 1.0)
        thisFeedItem.thumbnail = UIImageJPEGRepresentation(filterImage, 0.1)
    ...
    // Caching
    func cacheImage(imageNum: Int) {
        let filename = "\(thisFeedItem.uniqueID)\(imageNum)"
        let uniquePath = tmp.stringByAppendingPathComponent(filename)
       
        if !NSFileManager.defaultManager().fileExistsAtPath(filename) {
            let data = self.thisFeedItem.thumbnail
            let filter = self.filters[imageNum]
    ...
    func getCachedImage (imageNum: Int) -> UIImage {
        let fileName = "\(thisFeedItem.uniqueID)\(imageNum)"
        let uniquePath = tmp.stringByAppendingPathComponent(fileName)
        var image:UIImage
...
extension FilterViewController: UICollectionViewDataSource {
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        // !!!!!!! GCD !!!!!!!!!
        let filterQ:dispatch_queue_t = dispatch_queue_create("filterQ", nil)
        
        dispatch_async(filterQ, { () -> Void in
